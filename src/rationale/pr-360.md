...
In practice, rfcbot never supported advisor concerns, and we relied on an informal process of lang members proxying concerns without much difficulty. At the same time, the fact that advisors had the formal ability to block held us back in some cases from recognizing advisors that we otherwise wanted to. This change documents the de facto process while simplifying the formal process and removing the incentive to keep lang-advisors artificially small.

If in the future we find that advisor concerns are getting dropped or ignored, we can add a more formal process for them then. Such a process would have to deal with the challenges inherent in adding more "veto points" or procedural hacks and threatening the nature of the team as a deliberative, unified entity. Options include pausing an FCP until the lang team has time to discuss the concern in triage and decide on next steps. We recommend making sure our tools support the new process before documenting it to ensure our process docs stay normative and reliable.
